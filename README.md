# CSP User Interface Plugin

Plugin to ustomize Vue.js templates and components for the interface of the journal Cadernos de Saúde Pública - Open Journal System (OJS).

**OJS version: 3.5.0**

## Installation

1) Clone this repo inside the directory ``ojs/plugins/generic/`` :

   ``git clone --branch ojs-3.5 https://github.com/FiocruzLivre/ojs-csp-ui.git cspUI``

    > The plugin must be inside a _cspUI_ named folder and checked out to the ojs-3.5 branch

2) In the system, enable the plugin in _Website > Plugins_ area
